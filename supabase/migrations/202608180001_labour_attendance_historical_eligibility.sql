-- Preserve frozen Labour Attendance populations while allowing business-effective
-- eligibility for new Standard Attendance draft dates.
begin;
...
begin
  ...
  with eligible as (
    select d.*, w.labour_code, w.worker_name,
    ...
    where d.organization_id = v_period.organization_id
      ...
      and (
        (v_status = 'reopened' and (
          (v_previous_version_id is not null and d.id in (
            select deployment_id from public.labour_attendance_submission_version_rows
            where submission_version_id = v_previous_version_id and deployment_id is not null
          ))
          or (v_previous_version_id is null and d.id in (
            select distinct deployment_id from public.labour_attendance
    ...
    select *, case
      when first_half_present is null or second_half_present is null then 'incomplete'
  ...
  select count(*), count(*) filter (where derived_status = 'present'),
    ...
    coalesce(sum(coalesce(overtime_minutes, approved_overtime_minutes, 0)), 0),
  ...
  with eligible as (
    ...
    where d.organization_id = v_period.organization_id and d.company_id = v_period.company_id and d.site_id = v_period.site_id
      ...
      )) or (v_status = 'draft' and d.status in ('active','ended') and d.effective_from <= p_attendance_date
        and (d.effective_to is null or d.effective_to >= p_attendance_date)
        and (w.date_of_joining is null or w.date_of_joining <= p_attendance_date)
        and (w.date_of_exit is null or w.date_of_exit >= p_attendance_date)))
  )
  select v_snapshot_id, labour_worker_id, id, labour_code, worker_name, contractor_name, trade_name,
    commercial_model, first_half_present, second_half_present,
    case when first_half_present is null or second_half_present is null then 'incomplete'
      when first_half_present and second_half_present then 'present'
      when not first_half_present and not second_half_present then 'absent' else 'half_day' end,
  ...
  set summary = jsonb_set(coalesce(v_period.summary, '{}'::jsonb), array['date_statuses', p_attendance_date::text],
      coalesce(v_period.summary->'date_statuses'->p_attendance_date::text, '{}'::jsonb) ||
      jsonb_build_object('status','submitted','submitted_at',now(),'submitted_by',v_actor_id,
        'submitted_by_name',v_actor_name,'submitted_by_email',v_actor_email), true),
    submitted_by = v_actor_id, submitted_by_name = v_actor_name, submitted_by_email = v_actor_email,
    submitted_at = now(), updated_at = now()
  where id = p_period_id;

  return jsonb_build_object('submission_id', v_snapshot_id, 'submission_version', v_version,
    'eligible_worker_count', v_rows, 'present_count', v_present, 'absent_count', v_absent,
    'half_day_count', v_half_day, 'incomplete_count', v_incomplete,
    'overtime_minutes_total', v_ot, 'bonus_minutes_total', v_bonus);
end;
$$;
