...
-- Preserve the established transition body under a private, unambiguous name.
create or replace function public.transition_procurement_purchase_order_legacy_atomic(
...
begin
  ...
  if p_action = 'approve' then
    ...
    select e.* into v_employee from public.hr_employees e where e.user_id = nullif(p_actor->>'user_id','')::uuid and e.organization_id = v_po.organization_id and e.status = 'active' order by e.id limit 1;
    if not found then raise exception 'No active employee profile is configured for this approver.'; end if;
    select a.* into v_assignment from public.hr_employee_company_assignments a join public.hr_designations d on d.id = a.designation_id where a.employee_id = v_employee.id and a.organization_id = v_po.organization_id and a.company_id = v_po.company_id and a.status = 'active' and (a.effective_from is null or a.effective_from <= current_date) and (a.effective_to is null or a.effective_to >= current_date) and d.organization_id = v_po.organization_id and d.status = 'active';
  ...
  update public.procurement_purchase_orders set
    ...
    updated_at=now()
   where id=v_po.id;
  if p_action = 'approve' and v_previous.id is not null then update public.procurement_purchase_orders set superseded_by_revision_id=v_po.id where id=v_previous.id; end if;
...
revoke all on function public.transition_procurement_purchase_order_legacy_atomic(uuid,uuid,text,jsonb,text) from public, anon, authenticated;
...
begin
  ...
  return public.transition_procurement_purchase_order_legacy_atomic(
...
drop function public.transition_procurement_purchase_order_atomic(uuid,uuid,text,jsonb,text);
