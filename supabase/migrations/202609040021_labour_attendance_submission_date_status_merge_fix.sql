-- Fix Standard Labour Attendance submission date status JSON merge while preserving
-- the submitted-date mutation guard and snapshot integrity protections.
begin;

create or replace function public.labour_attendance_date_has_reopen_authority(
  p_period public.labour_attendance_periods,
...
as $$
begin
  if coalesce(p_period.summary->'date_statuses'->p_attendance_date::text->>'status', '') = 'reopened' then
    return true;
  end if;
  if p_period.status = 'reopened' then
    return true;
  end if;
  return false;
end;
...
create or replace function public.assert_labour_attendance_date_mutable(
  p_period_id uuid,
  p_attendance_date date
) returns void
language plpgsql
...
declare
  ...
  v_latest_submitted public.labour_attendance_submission_versions%rowtype;
begin
  if p_period_id is null or p_attendance_date is null then
    return;
  end if;
  ...
  if found and not public.labour_attendance_date_has_reopen_authority(v_period, p_attendance_date) then
    raise exception 'Attendance for this date has already been submitted. Reopen the date before making changes.';
  end if;
end;
$$;

create or replace function public.guard_labour_attendance_submitted_date_mutation()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if tg_op in ('UPDATE', 'DELETE') then
    perform public.assert_labour_attendance_date_mutable(old.period_id, old.attendance_date);
  end if;
  if tg_op in ('INSERT', 'UPDATE') then
    perform public.assert_labour_attendance_date_mutable(new.period_id, new.attendance_date);
  end if;
  return case when tg_op = 'DELETE' then old else new end;
end;
$$;

drop trigger if exists labour_attendance_submitted_date_mutation_guard on public.labour_attendance;
create trigger labour_attendance_submitted_date_mutation_guard
before insert or update or delete on public.labour_attendance
for each row execute function public.guard_labour_attendance_submitted_date_mutation();
...
begin
  ...
  select id into v_latest_submitted_id
  from public.labour_attendance_submission_versions
  ...
  if v_latest_submitted_id is not null and not public.labour_attendance_date_has_reopen_authority(v_period, p_attendance_date) then
    raise exception 'Attendance for this date has already been submitted. Reopen the date before making changes.';
  end if;
      ...
      || jsonb_build_object(
        ...
        || jsonb_build_object(
          ...
          || jsonb_build_object(
            ...
            'submitted_by_name',v_actor_name,
            'submitted_by_email',v_actor_email
          )
        )
      ),
      submitted_by = v_actor_id,
...
revoke all on function public.labour_attendance_date_has_reopen_authority(public.labour_attendance_periods, date) from public, anon, authenticated;
revoke all on function public.assert_labour_attendance_date_mutable(uuid, date) from public, anon, authenticated;
revoke all on function public.guard_labour_attendance_submitted_date_mutation() from public, anon, authenticated;
