-- V2 item identity support layered on existing PO live-master/atomic-freeze behavior.
begin;
...
CREATE OR REPLACE FUNCTION public.create_procurement_purchase_order_draft_idempotent_v2_atomic(
  p_source_type text,
  p_organization_id uuid,
  p_company_id uuid,
  p_site_id uuid,
...
begin
  ...
  v_result :=
    public.create_procurement_purchase_order_draft_v2_atomic(
      p_source_type,
      p_organization_id,
      p_company_id,
      p_site_id,
...
revoke all on function public.create_procurement_purchase_order_draft_idempotent_v2_atomic(text, uuid, uuid, uuid, uuid, jsonb, uuid, text, jsonb, jsonb, jsonb, uuid) from public;
grant execute on function public.create_procurement_purchase_order_draft_idempotent_v2_atomic(text, uuid, uuid, uuid, uuid, jsonb, uuid, text, jsonb, jsonb, jsonb, uuid) to authenticated, service_role;
