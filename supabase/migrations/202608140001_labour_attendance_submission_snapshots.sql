...
create table if not exists public.labour_attendance_submission_versions (
  ...
  status text not null default 'submitted' check (status in ('submitted', 'reopened', 'superseded')),
  submitted_by uuid,
  ...
  bonus_minutes_total integer not null default 0,
  created_at timestamptz not null default now(),
  unique (period_id, attendance_date, submission_version)
);

create table if not exists public.labour_attendance_submission_version_rows (
  id uuid primary key default gen_random_uuid(),
  submission_version_id uuid not null references public.labour_attendance_submission_versions(id) on delete restrict,
  labour_worker_id uuid references public.labour_workers(id),
  deployment_id uuid references public.labour_deployments(id),
  labour_code_snapshot text not null,
  worker_name_snapshot text not null,
  contractor_name_snapshot text,
  trade_snapshot text,
  commercial_model_snapshot text,
  first_half_present boolean,
  second_half_present boolean,
  derived_status text not null check (derived_status in ('present', 'absent', 'half_day', 'incomplete')),
...
create unique index if not exists labour_attendance_submission_versions_scope_uidx
  on public.labour_attendance_submission_versions (period_id, attendance_date, submission_version);
create index if not exists labour_attendance_submission_versions_current_idx
  on public.labour_attendance_submission_versions (period_id, attendance_date, status, submission_version desc);
create index if not exists labour_attendance_submission_version_rows_submission_idx
  on public.labour_attendance_submission_version_rows (submission_version_id);

alter table public.labour_attendance_submission_versions enable row level security;
alter table public.labour_attendance_submission_version_rows enable row level security;
revoke all on table public.labour_attendance_submission_versions, public.labour_attendance_submission_version_rows from public, anon, authenticated;
grant select, insert on table public.labour_attendance_submission_versions to service_role;
grant select, insert on table public.labour_attendance_submission_version_rows to service_role;
...
begin
  ...
  if coalesce(v_period.summary->'date_statuses'->p_attendance_date::text->>'status', v_period.status) not in ('draft','reopened') then
  ...
  update public.labour_attendance_periods set summary = jsonb_set(coalesce(v_period.summary,'{}'::jsonb), array['date_statuses', p_attendance_date::text], coalesce(v_period.summary->'date_statuses'->p_attendance_date::text,'{}'::jsonb) || jsonb_build_object('status','submitted','submitted_at',now(),'submitted_by',v_actor_id,'submitted_by_name',v_actor_name,'submitted_by_email',v_actor_email), true), submitted_by = v_actor_id, submitted_by_name = v_actor_name, submitted_by_email = v_actor_email, submitted_at = now(), updated_at = now() where id = p_period_id;
