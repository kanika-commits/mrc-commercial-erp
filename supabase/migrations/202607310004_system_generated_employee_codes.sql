create sequence if not exists public.employee_code_sequence start with 5 increment by 1;

create table if not exists public.employee_code_reservations (
  id uuid primary key default gen_random_uuid(),
  reserved_code text not null unique,
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'employee_code_reservations_assignment_consistency'
      and conrelid = 'public.employee_code_reservations'::regclass
  ) then
    alter table public.employee_code_reservations
      add constraint employee_code_reservations_assignment_consistency
      check (
        (is_assigned = false and assigned_employee_id is null)
        or
        (is_assigned = true and assigned_employee_id is not null)
      );
  end if;
end $$;

alter table public.employee_code_reservations enable row level security;

revoke all on table public.employee_code_reservations from anon;
revoke all on table public.employee_code_reservations from authenticated;
grant all on table public.employee_code_reservations to service_role;

insert into public.employee_code_reservations (reserved_code, display_name, notes)
values
  ('MRC0001', 'Mukesh Ranjan', 'Management-reserved employee code.'),
  ('MRC0002', 'Sunita Ranjan', 'Management-reserved employee code.'),
  ('MRC0003', 'Raghav Ranjan', 'Management-reserved employee code.'),
  ('MRC0004', 'Kanika Puri', 'Management-reserved employee code.')
on conflict (reserved_code) do update
set display_name = excluded.display_name,
    notes = excluded.notes,
    updated_at = now();

create or replace function public.format_employee_code(p_number bigint)
returns text
language sql
immutable
as $$
  select 'MRC' || lpad(p_number::text, 4, '0')
$$;

create or replace function public.is_reserved_employee_code(p_code text)
returns boolean
language sql
stable
as $$
  select exists (
    select 1
    from public.employee_code_reservations
    where upper(trim(reserved_code)) = upper(trim(p_code))
  )
$$;
...
begin
  loop
    v_number := nextval('public.employee_code_sequence');
    v_code := public.format_employee_code(v_number);

    exit when not public.is_reserved_employee_code(v_code)
      and not exists (
        select 1
        from public.hr_employees
        where upper(trim(employee_code)) = upper(trim(v_code))
      );
  end loop;
...
revoke all on function public.format_employee_code(bigint) from public;
revoke all on function public.format_employee_code(bigint) from anon;
revoke all on function public.format_employee_code(bigint) from authenticated;
grant execute on function public.format_employee_code(bigint) to service_role;

revoke all on function public.is_reserved_employee_code(text) from public;
revoke all on function public.is_reserved_employee_code(text) from anon;
revoke all on function public.is_reserved_employee_code(text) from authenticated;
grant execute on function public.is_reserved_employee_code(text) to service_role;

revoke all on function public.next_employee_code() from public;
revoke all on function public.next_employee_code() from anon;
revoke all on function public.next_employee_code() from authenticated;
...
with ordered as (
  select
    id,
    row_number() over (
      order by date_of_joining asc nulls last, created_at asc, id asc
    ) + 4 as seq_no
  from public.hr_employees
...
temporary_codes as (
  ...
  set employee_code = '__TMP_EMP_' || e.id::text
  from ordered o
  where e.id = o.id
  returning e.id
)
update public.hr_employees e
set employee_code = public.format_employee_code(o.seq_no)
from ordered o
where e.id = o.id;
...
select setval(
  ...
  greatest(
    5,
    coalesce((
      select max((regexp_match(employee_code, '^MRC([0-9]+)$'))[1]::bigint)
      from public.hr_employees
      where employee_code ~ '^MRC[0-9]+$'
    ), 4) + 1
  ),
  false
);
...
begin
  ...
  begin
    v_employee_code := public.next_employee_code();
    ...
    if v_has_salary and to_regclass('public.employee_salary_history') is not null then
      if v_has_joining_salary then
        ...
        using v_employee.organization_id, v_employee.id,
          ...
          p_batch_id::text || ':' || v_row.source_row_number::text, p_batch_id,
    ...
    v_result := jsonb_build_object(
      ...
      'message', 'Employee imported. Employee Code: ' || v_employee.employee_code
    );
...
revoke all on function public.execute_employee_import_row(uuid, uuid, uuid, text, text) from public;
revoke all on function public.execute_employee_import_row(uuid, uuid, uuid, text, text) from anon;
revoke all on function public.execute_employee_import_row(uuid, uuid, uuid, text, text) from authenticated;
grant execute on function public.execute_employee_import_row(uuid, uuid, uuid, text, text) to service_role;
