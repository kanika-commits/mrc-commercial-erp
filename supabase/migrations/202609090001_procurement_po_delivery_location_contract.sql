...
begin
  ...
  if v_delivery.billing_address_id is not null then

    select *
      into v_billing
      from public.company_billing_addresses
     where id = v_delivery.billing_address_id
       and organization_id = p_organization_id
       and company_id = p_company_id
       and status = 'active';

    if v_billing.id is null then
      raise exception 'Selected Delivery Location has an inactive, outside-organization, or incompatible Billing Address.';
    end if;

    if v_billing.gst_registration_id is not null then
      select *
        into v_gst
        from public.company_gst_registrations
       where id = v_billing.gst_registration_id
         and organization_id = p_organization_id
         and company_id = p_company_id
         and status = 'active';

      if v_gst.id is null then
        raise exception 'Selected Delivery Location has an inactive, outside-organization, or incompatible GST Registration.';
      end if;
    else
      v_gst := null;
    end if;
  ...
  return jsonb_build_object(
    ...
    jsonb_build_object(
      ...
      'billing_address_id', v_billing.id,
      'company_id', p_company_id,
      'gstin', upper(btrim(coalesce(v_gst.gstin, v_billing.gstin))),
