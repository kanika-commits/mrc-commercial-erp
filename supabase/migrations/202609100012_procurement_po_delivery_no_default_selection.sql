...
declare
  ...
  v_gst_registration_id uuid := nullif(v_selection->>'gst_registration_id', '')::uuid;

  v_billing_address_id uuid := nullif(v_selection->>'billing_address_id', '')::uuid;
...
begin
  ...
  into
    v_gst_count
  from public.company_gst_registrations g
  where g.organization_id = p_organization_id
    and g.company_id = p_company_id
    and g.status = 'active'
    and exists (select 1 from public.company_billing_addresses b where b.organization_id = g.organization_id and b.company_id = g.company_id and b.gst_registration_id = g.id and b.status = 'active');

  if v_gst_count = 0 then
    raise exception 'Configure an active GST/Billing Master for the selected Company.';

  elsif v_gst_registration_id is not null then

    select *
      into v_gst
      from public.company_gst_registrations g
     where g.id = v_gst_registration_id
    ...
    if v_gst.id is null then
      raise exception 'Selected GST/Billing Master is inactive or unavailable.';
    end if;
    ...
    raise exception 'Select a GST/Billing Master for the selected Company.';
  end if;
  ...
  if v_billing_address_id is not null then
    select * into v_billing
      from public.company_billing_addresses
     where id = v_billing_address_id and organization_id = p_organization_id and company_id = p_company_id
       and status = 'active' and gst_registration_id = v_gst.id;
    if v_billing.id is null then raise exception 'Selected Billing Address is inactive or not linked to the selected GST/Billing Master.'; end if;
  elsif v_billing_count = 1 then
    select * into v_billing
      from public.company_billing_addresses
     where organization_id = p_organization_id and company_id = p_company_id and status = 'active' and gst_registration_id = v_gst.id;
  else
    raise exception 'Select a Billing Address for the selected GST/Billing Master.';
  end if;
