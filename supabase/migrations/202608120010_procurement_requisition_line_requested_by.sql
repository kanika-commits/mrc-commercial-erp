  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    ...
    select e.employee_name into v_requested_by_name
     ...
     where e.id = nullif(v_item->>'requested_by_employee_id', '')::uuid
       ...
       and e.company_id = (p_header->>'company_id')::uuid
       and e.status = 'active'
       and (e.site_id = (p_header->>'site_id')::uuid or exists (
         select 1 from public.sites ho
          where ho.id = e.site_id and ho.organization_id = e.organization_id
            and ho.company_id = e.company_id and ho.status = 'active' and ho.site_code = 'HO'
       ));
...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    ...
    select e.employee_name into v_requested_by_name from public.hr_employees e where e.id = nullif(v_item->>'requested_by_employee_id', '')::uuid and e.organization_id = v_requisition.organization_id and e.company_id = v_target_company_id and e.status = 'active' and (e.site_id = (p_header->>'site_id')::uuid or exists (select 1 from public.sites ho where ho.id = e.site_id and ho.organization_id = e.organization_id and ho.company_id = e.company_id and ho.status = 'active' and ho.site_code = 'HO'));
    if not found then raise exception 'Requested-by employee is invalid on a material line.'; end if;
  end loop;
  if exists (
    select 1
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    v_index := v_index + 1;
    v_line_key := coalesce(nullif(v_item->>'line_key', '')::uuid, gen_random_uuid());
    select e.employee_name into v_requested_by_name from public.hr_employees e where e.id = nullif(v_item->>'requested_by_employee_id', '')::uuid and e.organization_id = v_requisition.organization_id and e.company_id = v_target_company_id and e.status = 'active' and (e.site_id = v_target_site_id or exists (select 1 from public.sites ho where ho.id = e.site_id and ho.organization_id = e.organization_id and ho.company_id = e.company_id and ho.status = 'active' and ho.site_code = 'HO'));
    v_requested_uom_id := nullif(v_item->>'uom_id', '')::uuid;
    v_quantity := (v_item->>'quantity')::numeric;
...
begin
  ...
  for item_json in select value from jsonb_array_elements(p_items) loop
    ...
    select e.employee_name into requested_by_name from public.hr_employees e where e.id = nullif(item_json->>'requested_by_employee_id', '')::uuid and e.organization_id = requisition_row.organization_id and e.company_id = requisition_row.company_id and e.status = 'active' and (e.site_id = requisition_row.site_id or exists (select 1 from public.sites ho where ho.id = e.site_id and ho.organization_id = e.organization_id and ho.company_id = e.company_id and ho.status = 'active' and ho.site_code = 'HO'));
    if not found then raise exception 'Requested-by employee is invalid on a material line.'; end if;
  end loop;
  if exists (
    select 1
