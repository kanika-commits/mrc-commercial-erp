...
create table if not exists public.employee_import_batches (
  ...
  executed_by_name text null,
  executed_by_email text null,
  executed_at timestamptz null,
  constraint employee_import_batches_status_check
    check (status in ('draft', 'validated', 'ready', 'executing', 'completed', 'completed_with_errors', 'failed', 'cancelled'))
);

create index if not exists idx_employee_import_batches_org_created
  on public.employee_import_batches (organization_id, created_at desc);

create index if not exists idx_employee_import_batches_status
  on public.employee_import_batches (status, created_at desc);
...
create table if not exists public.employee_import_rows (
  ...
  matched_site_id uuid null references public.sites(id) on delete set null,
  matched_department_id uuid null references public.hr_departments(id) on delete set null,
  matched_designation_id uuid null references public.hr_designations(id) on delete set null,
  matched_employee_id uuid null references public.hr_employees(id) on delete set null,
  imported_employee_id uuid null references public.hr_employees(id) on delete set null,
  import_result jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  imported_at timestamptz null,
  constraint employee_import_rows_mapping_status_check
    check (mapping_status in ('unmapped', 'mapped', 'needs_review')),
  constraint employee_import_rows_validation_status_check
    check (validation_status in ('pending', 'valid', 'invalid', 'warning')),
  constraint employee_import_rows_import_status_check
    check (import_status in ('pending', 'imported', 'skipped', 'failed'))
);

create unique index if not exists uniq_employee_import_rows_batch_source_row
  on public.employee_import_rows (batch_id, source_row_number);

create unique index if not exists uniq_employee_import_rows_batch_hash
  on public.employee_import_rows (batch_id, source_row_hash);

create index if not exists idx_employee_import_rows_batch_status
  on public.employee_import_rows (batch_id, validation_status, import_status);

create index if not exists idx_employee_import_rows_employee
  on public.employee_import_rows (imported_employee_id);

create table if not exists public.employee_compliance_records (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid null references public.organizations(id) on delete set null,
  employee_id uuid not null references public.hr_employees(id) on delete cascade,
  import_batch_id uuid null references public.employee_import_batches(id) on delete set null,
  import_row_id uuid null references public.employee_import_rows(id) on delete set null,
  ...
  updated_by_email text null,
  updated_at timestamptz not null default now(),
  constraint employee_compliance_records_source_check
    check (source in ('manual', 'import', 'system')),
  constraint employee_compliance_records_status_check
    check (status in ('active', 'inactive', 'deleted'))
);

create unique index if not exists uniq_employee_compliance_records_import_row_type
  on public.employee_compliance_records (import_row_id, record_type)
  where import_row_id is not null;

create index if not exists idx_employee_compliance_records_employee
  on public.employee_compliance_records (employee_id, record_type, status);

create index if not exists idx_employee_compliance_records_org
  on public.employee_compliance_records (organization_id, created_at desc);
...
begin
  if to_regclass('public.erp_modules') is not null then
    ...
    select
      ...
      'active'
    where not exists (
      select 1 from public.erp_modules where module_code = 'hr_employee_import'
    );
  end if;
end $$;
    ...
    where roles.role_code in ('super_admin')
      and not exists (
        ...
        where existing.role_id = roles.id
          and existing.module_code = 'hr_employee_import'
...
begin
  if exists (select 1 from pg_roles where rolname = 'authenticated') then
    grant select, insert, update, delete on table public.employee_import_batches to authenticated;
    grant select, insert, update, delete on table public.employee_import_rows to authenticated;
    grant select, insert, update, delete on table public.employee_compliance_records to authenticated;
  end if;

  if exists (select 1 from pg_roles where rolname = 'service_role') then
    grant select, insert, update, delete on table public.employee_import_batches to service_role;
    grant select, insert, update, delete on table public.employee_import_rows to service_role;
    grant select, insert, update, delete on table public.employee_compliance_records to service_role;
  end if;
end $$;
  ...
  begin
    ...
    if to_regclass('public.employee_employment_history') is not null then
      ...
      using v_employee.organization_id, v_employee.id, v_joining_date, to_jsonb(v_employee),
        ...
        v_employee.employment_type, v_employee.shift, v_employee.status, v_row.source_row_number::text,
...
begin
  if exists (select 1 from pg_roles where rolname = 'authenticated') then
    grant execute on function public.execute_employee_import_row(uuid, uuid, uuid, text, text) to authenticated;
  end if;
