    ...
    <section className="space-y-6">
      ...
      {loading ? (
        ...
        <section className="rounded-2xl border bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">Company Assignments</h2><p className="mt-1 text-sm text-slate-500">The existing Company and Designation fields remain the employee&apos;s primary HR assignment. Use this list for additional company-specific assignments.</p></div><button type="button" onClick={() => { setAssignmentForm({ id: "", company_id: "", designation_id: "", status: "active" }); setAssignmentOpen(true); }} className="rounded-lg border px-3 py-2 text-sm font-semibold">+ Add Company Assignment</button></div>
          {assignmentOpen && <div className="mt-5 grid gap-3 rounded-xl border bg-slate-50 p-4 md:grid-cols-4"><select value={assignmentForm.company_id} onChange={(e) => setAssignmentForm((v) => ({ ...v, company_id: e.target.value }))} className="h-10 rounded-lg border bg-white px-3"><option value="">Company</option>{lookups.companies.map((row) => <option key={row.id} value={row.id}>{row.label}</option>)}</select><select value={assignmentForm.designation_id} onChange={(e) => setAssignmentForm((v) => ({ ...v, designation_id: e.target.value }))} className="h-10 rounded-lg border bg-white px-3"><option value="">Designation</option>{lookups.designations.map((row) => <option key={row.id} value={row.id}>{row.designation_name}</option>)}</select><select value={assignmentForm.status} onChange={(e) => setAssignmentForm((v) => ({ ...v, status: e.target.value }))} className="h-10 rounded-lg border bg-white px-3"><option value="active">Active</option><option value="inactive">Inactive</option></select><div className="flex gap-2"><button type="button" onClick={saveAssignment} className="rounded-lg bg-slate-900 px-3 py-2 text-sm font-semibold text-white">Save</button><button type="button" onClick={() => setAssignmentOpen(false)} className="rounded-lg border px-3 py-2 text-sm font-semibold">Cancel</button></div></div>}
