...
export default function EditCompanyBankAccountPage() {
  ...
  async function loadData() {
    try {
      ...
      if (!accountResponse.ok) {
        throw new Error(accountResult.error || "Failed to load bank account.");
      }

      if (companiesResponse.error) throw companiesResponse.error;

      const account = accountResult.account;
      setCompanies(sortCompanies(companiesResponse.data || []));
      setForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/company-bank-accounts/${accountId}`);
    } catch (error: any) {
      setMessage(error.message || "Failed to update bank account.");
    } finally {
      setSaving(false);
    }
  }
    ...
    return (
      <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-red-700">
        ...
        <p className="mt-1 text-sm">
          You do not have permission to edit company bank accounts.
        </p>
        <Link
          href={`/company-bank-accounts/${accountId}`}
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Update company-wise payment account details.
          </p>
        </div>
      ...
      <section className="rounded-2xl border bg-white p-6 shadow-sm">
        ...
        {loading ? (
          <div className="mt-4 text-sm text-slate-500">Loading bank account...</div>
        ) : (
