...
export default function DebitNotesPage() {
    ...
    try {
      ...
      const { data: noteData, error: noteError } = await supabase
        ...
        .select(`
          ...
          created_at
        `)
        .in("approval_status", ["Approved", "approved", "Rejected", "rejected"])
  ...
  const approvedNotes = notes.filter(
    (note) => String(note.approval_status || "").toLowerCase() !== "rejected"
  );
  ...
  const totalDebitValue = approvedNotes.reduce(
    (sum: number, note: any) =>
  ...
  if (loading) {
    return <p className="text-sm text-slate-500">Loading Debit Notes...</p>;
  }
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Approved debit notes are shown separately from rejected debit notes.
          </p>
        </div>
                  ...
                  <tr key={note.id} className="border-t hover:bg-slate-50">
                    ...
                    <td className="p-3">
                      ...
                      {note.approved_by_name &&
                        ...
                        note.approved_by_name !== note.approved_by_email && (
                          <div className="max-w-[180px] truncate text-xs text-slate-500">
                            {note.approved_by_email}
                          </div>
                        )}
      ...
      {rejectedNotes.length > 0 && (
        <div className="rounded-2xl border border-red-200 bg-white shadow-sm">
          <div className="border-b border-red-100 p-4">
            <h2 className="font-semibold text-red-700">Rejected Debit Notes</h2>
            <p className="text-xs text-slate-500">
              Rejected debit notes are shown separately and are not included in approved totals.
            </p>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1000px] text-sm">
              ...
              <tbody>
                {rejectedNotes.map((note: any) => {
                  const wo = woMap.get(note.work_order_id);
                  const vendor = vendorMap.get(note.vendor_id);

                  return (
                    <tr key={note.id} className="border-t border-red-100">
                      <td className="p-3">{note.debit_note_date || "-"}</td>
                      <td className="p-3 font-semibold">{note.debit_note_number}</td>
                      <td className="p-3">{vendor?.vendor_name || "-"}</td>
