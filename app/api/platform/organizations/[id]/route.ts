...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [organization, modules, domains, memberships, companies, sites] = await Promise.all([
    ...
    if (modules.error || domains.error || memberships.error || companies.error || sites.error) throw modules.error || domains.error || memberships.error || companies.error || sites.error;
    return NextResponse.json({ organization: organization.data, entitlementMode: (modules.data || []).length ? "Managed" : "Legacy", modules: modules.data || [], domains: domains.data || [], memberships: memberships.data || [], businessSummary: { companies: companies.count || 0, sites: sites.count || 0 } });
  } catch (error: any) {
