...
export async function GET(request: Request) {
  try {
    ...
    const singleId = searchParams.get("ra_bill_id")?.trim();
    const ids = (searchParams.get("ra_bill_ids") || "")
      .split(",")
      .map((id) => id.trim())
      .filter(Boolean);
    const billIds = singleId ? [singleId] : Array.from(new Set(ids));

    if (billIds.length === 0) {
      return NextResponse.json(
        { error: "ra_bill_id or ra_bill_ids is required." },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { error: error.message || "Failed to load RA Bill documents." },
      { status: 500 }
    );
  }
}
