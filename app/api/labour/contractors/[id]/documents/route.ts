...
import {
  ...
  loadScopedContractor,
  requireLabourPermission,
} from "@/app/api/labour/_shared";
import { LABOUR_CONTRACTOR_DOCUMENT_TYPES, normalizeText } from "@/lib/labour/constants";
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return jsonError(error.message || "Failed to load contractor documents.", 500);
  }
}
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!documentType || !LABOUR_CONTRACTOR_DOCUMENT_TYPES.includes(documentType as any)) return jsonError("Valid document type is required.");
    ...
    const key = safeObjectKey([contractor.organization_id, "contractors", id, documentType, `${Date.now()}-${file.name}`]);
    ...
    const { data, error } = await access.admin.from("labour_contractor_documents").insert(insertPayload).select("id").single();
    if (error) throw error;
    ...
    await audit(access, request, {
      ...
      entityType: "labour_contractor_document",
      recordId: data.id,
      parentEntityType: "labour_contractor_profile",
      parentRecordId: id,
    ...
    return jsonError(error.message || "Failed to upload contractor document.", 500);
  }
}
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: document, error } = await access.admin.from("labour_contractor_documents").select("*").eq("id", documentId).eq("contractor_profile_id", id).maybeSingle();
    if (error) throw error;
    if (!document) return jsonError("Document not found.", 404);
    await createPrivateStorageAdapter(access.admin).delete({ bucket: document.storage_bucket, key: document.storage_key });
    const { error: deleteError } = await access.admin.from("labour_contractor_documents").delete().eq("id", documentId);
    ...
    return jsonError(error.message || "Failed to delete contractor document.", 500);
  }
}
