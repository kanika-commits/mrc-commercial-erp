...
async function notifyCurrentRequisitionApprovers(admin: any, row: any, actorId: string) {
  try {
    const { data: states, error } = await admin.from("purchase_requisition_line_approval_state")
      .select("requisition_item_line_key, workflow_version, submission_cycle, current_approval_layer, approval_status")
      .eq("requisition_id", row.id).eq("approval_status", "pending");
    if (error) throw error;
    const pending = states || [];
    const keys = pending.map((s: any) => s.requisition_item_line_key).filter(Boolean);
    if (!keys.length) return;
    const { data: steps, error: stepError } = await admin.from("purchase_requisition_line_approval_steps")
      .select("requisition_item_line_key, workflow_version, submission_cycle, layer_number, approver_user_id, status")
      .eq("requisition_id", row.id).in("requisition_item_line_key", keys).eq("status", "pending");
    if (stepError) throw stepError;
    const actionable = (steps || []).filter((step: any) => pending.some((state: any) => state.requisition_item_line_key === step.requisition_item_line_key && state.workflow_version === step.workflow_version && state.submission_cycle === step.submission_cycle && Number(state.current_approval_layer) === Number(step.layer_number)));
    const recipientIds = actionable.map((step: any) => step.approver_user_id).filter(Boolean);
    const stage = Array.from(new Set(actionable.map((step: any) => String(step.layer_number)))).sort().join(",") || "none";
    const cycleId = Array.from(new Set(actionable.map((step: any) => `${step.workflow_version}:${step.submission_cycle}:${step.layer_number}`))).sort().join("|") || row.id;
    await notifyWorkflowRecipients(admin, {
      eventType: "material_indent_awaiting_approval",
      ...
      siteId: row.site_id,
      title: "Material Indent awaiting approval",
      message: `${row.requisition_number} is waiting for your approval.`,
      targetUrl: `/purchase/requisitions/${encodeURIComponent(row.id)}?from=approval`,
      recipientIds,
      actorId,
      stage,
      requiredPermission: { moduleCode: APPROVAL_MODULE, actionCode: "approve" },
    });
  } catch (error) {
    console.error("Material Indent notification delivery failed:", error);
  }
}
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!row) return jsonError("Requisition was not found.", 404);
    ...
    if (row.procurement_flow === "billing_engineer") {
      ...
      await notifyCurrentRequisitionApprovers(admin, row, auth.user.id);
