...
export async function PUT(request: Request) {
  try {
    ...
    const result = await admin.from("procurement_company_letterhead_versions").update(patch).eq("id", versionId).eq("version_status", "draft"); if (result.error) throw result.error; return NextResponse.json({ ok: true });
...
export async function PATCH(request: Request) {
  try {
    ...
    if (action === "ready") { if (version.version_status !== "draft" || !version.header_storage_key || !version.footer_storage_key || !version.header_content_hash || !version.footer_content_hash || !version.header_width_px || !version.footer_width_px) return jsonError("Upload valid header and footer PNG files before marking Ready.", 400); const result = await admin.from("procurement_company_letterhead_versions").update({ version_status: "ready" }).eq("id", version.id).eq("version_status", "draft"); if (result.error) throw result.error; return NextResponse.json({ ok: true }); }
