...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const metadata = copiedObjects.map((o, i) => ({ ...o, source_document_id: eligible[i].id, original_file_name: eligible[i].original_file_name || o.originalFileName, storage_provider: o.provider, storage_bucket: o.bucket, storage_key: o.key, mime_type: eligible[i].mime_type || o.mimeType, size_bytes: eligible[i].size_bytes ?? o.sizeBytes, document_type: eligible[i].document_type, sort_order: eligible[i].sort_order }));
