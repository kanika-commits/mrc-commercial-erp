        ...
        <tbody className="divide-y">
          {filtered.length === 0 ? <tr><td colSpan={11} className="p-8 text-center text-slate-500">No Purchase Orders found.</td></tr> : filtered.map((row, index) => (
            <tr key={row.id}>
              ...
              <td className="p-4"><span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${statusClass(row.status)}`}>{statusLabel(row.status)}</span></td>
