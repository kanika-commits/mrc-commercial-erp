...
export default function PurchaseOrdersPage() {
  ...
  const [revisionCreatingId, setRevisionCreatingId] = useState<string | null>(null);
  ...
  function canCreateRevision(row: any) { return row.status === "approved" && !row.superseded_by_revision_id && !row.revision_history?.some((candidate: any) => candidate.id !== row.id && ["draft", "pending_approval", "sent_back"].includes(candidate.status)); }
  async function createRevision(row: any) { setRevisionCreatingId(row.id); try { const result = await apiFetch(`/api/procurement/purchase-orders/${row.id}/revision`, { method: "POST", body: JSON.stringify({}) }); const nextId = result?.purchase_order_id || result?.id; if (!nextId) throw new Error("Revision was created without a purchase order id."); router.push(`/purchase/purchase-orders/new?edit_id=${nextId}`); } catch (error: any) { setMessage(error.message || "Unable to create revision."); } finally { setRevisionCreatingId(null); } }
        ...
        <tbody className="divide-y">
          {filtered.length === 0 ? <tr><td colSpan={11} className="p-8 text-center text-slate-500">No Purchase Orders found.</td></tr> : filtered.map((row, index) => (
            <tr key={row.id}>
              ...
              <td className="p-4">{row.po_date}</td><td className="p-4">{row.company?.company_name || "—"}</td><td className="p-4">{row.site?.site_name || "—"}</td><td className="p-4">{row.vendor_name_snapshot}</td><td className="p-4">{sourceLabel(row.source_type)}</td><td className="p-4 font-semibold">₹ {Number(row.total_amount || 0).toLocaleString("en-IN")}</td>
              <td className="p-4"><div className="flex min-w-48 flex-col items-start gap-1"><span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${statusClass(row.status)}`}>{statusLabel(row.status)}</span>{row.status === "sent_back" && row.latest_send_back_comment && <p className="line-clamp-2 max-w-64 text-xs leading-4 text-red-700" title={String(row.latest_send_back_comment)} aria-label={`Send-back reason: ${String(row.latest_send_back_comment)}`}>↳ {row.latest_send_back_comment}</p>}</div></td>
              <td className="p-4"><div className="flex items-center gap-2 whitespace-nowrap"><Link href={`/purchase/purchase-orders/${row.id}`} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border p-2" title="Open Purchase Order"><Eye className="h-4 w-4" /></Link>{canCreateRevision(row) && <button type="button" disabled={revisionCreatingId === row.id} onClick={() => void createRevision(row)} className="inline-flex h-9 items-center justify-center rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-900" title="Create a new revision of this approved PO" aria-label="Create a new revision of this approved PO">+ Revised PO</button>}{row.status === "draft" && <button type="button" onClick={() => setDeleteTarget(row)} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-red-200 p-2 text-red-700" title="Delete Draft Purchase Order"><Trash2 className="h-4 w-4" /></button>}</div></td>
              <td className="p-4">{!["approved", "issued"].includes(row.status) ? <span className="text-slate-400">—</span> : <div className="flex items-center gap-2 whitespace-nowrap">{row.signed_po_document ? <><span className="font-semibold text-emerald-700">Vendor Acceptance Received</span><a href={row.signed_po_document.signed_url || "#"} target="_blank" rel="noreferrer" className="inline-flex h-9 w-9 items-center justify-center rounded-lg border p-2" title="View Vendor Acceptance"><ExternalLink className="h-4 w-4" /></a><label className="inline-flex h-9 w-9 cursor-pointer items-center justify-center rounded-lg border p-2" title="Replace Vendor Acceptance"><Upload className="h-4 w-4" /><input type="file" className="hidden" accept="application/pdf,image/jpeg,image/png,image/webp" onChange={(event) => void uploadSignedPo(row, event)} /></label></> : <div className="flex flex-col items-start gap-1.5"><span className="inline-flex items-center gap-1 font-semibold text-amber-700"><TriangleAlert className="h-3.5 w-3.5" />Vendor Acceptance Pending</span><label className="inline-flex cursor-pointer items-center gap-1 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800" title="Upload Vendor Acceptance"><Upload className="h-3.5 w-3.5" />Upload<input type="file" className="hidden" accept="application/pdf,image/jpeg,image/png,image/webp" onChange={(event) => void uploadSignedPo(row, event)} /></label></div>}</div>}</td>
            </tr>
          ))}
        </tbody>
      </table>
