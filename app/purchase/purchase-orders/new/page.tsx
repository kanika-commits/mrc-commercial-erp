...
import { normalizePurchaseOrderItemSpecification, visiblePurchaseOrderItemSpecification } from "@/lib/procurement/poItemSpecification";
...
export default function NewPurchaseOrderPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([apiFetch(`/api/procurement/purchase-orders/${editId}`), apiFetch(`/api/procurement/purchase-orders/${editId}/documents`)]).then(([result, documentResult]) => {
      ...
      originalPersistedItemIdsRef.current = (po.items || []).map((item: any) => String(item.id));
  ...
  useEffect(() => {
    ...
    setItems((current) => current.map((item) => {
      const material = lookups.items.find((row: any) => row.id === item.item_id || row.item_code === item.item_code);
      return material ? { ...item, item_id: material.id, item_code: material.item_code, uom: material.default_uom?.uom_code || "", isMaterialMasterLinked: true } : item;
    }));
  }, [editId, lookups.items]);
  ...
  function chooseIndent(line: any) {
    ...
    setItems((current) => {
      ...
      return [...existing, { item_id: line.item_id, item_name: line.item_name_snapshot || "", item_code: line.item_code_snapshot, description: line.specification, quantity: String(line.remaining_quantity), uom: canonicalUomCode(line.item_id, line.uom_snapshot), unit_rate: "", gst_rate: "0", source_requisition_line_key: line.line_key, isMaterialMasterLinked: Boolean(line.item_id) }];
    });
  }
  ...
  function canonicalUomCode(itemId?: string, fallback?: string) {
    const material = (lookups.items || []).find((row: any) => row.id === itemId);
    return material?.default_uom?.uom_code || fallback || "";
  }
  ...
  function selectMaterial(index: number, material: any) {
    setItems((current) => current.map((item, itemIndex) => itemIndex === index ? {
      ...
      uom: material.default_uom?.uom_code || "",
      isMaterialMasterLinked: true,
  ...
  function useCustomItem(index: number) {
    setItems((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, item_id: undefined, item_code: undefined, uom: item.uom || "", source_requisition_line_key: undefined, isMaterialMasterLinked: false } : item));
    setMaterialOpenIndex(null);
    setMaterialMenuPosition(null);
  }
  ...
  function openMaterialMenu(index: number, input?: HTMLInputElement | null) {
    ...
    if (target) {
      ...
      setMaterialMenuPosition({ index, left: rect.left, top: rect.bottom + 10, width: Math.min(Math.max(rect.width, 280), 420) });
    }
  }
