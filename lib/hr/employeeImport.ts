...
export async function executeImportRow(
  ...
  if (codeError) {
    const message = String(codeError.message || "");
    if (codeError.code === "PGRST202" || codeError.code === "42883" || message.toLowerCase().includes("next_employee_code")) {
      throw new Error("Employee code generator is not available. Apply the employee-code migration before importing employees.");
    }
    throw codeError;
  }
