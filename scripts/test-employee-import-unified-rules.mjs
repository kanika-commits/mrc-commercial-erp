...
const templateText = fs.readFileSync(new URL("../outputs/employee-import-template/ConstructIQ_Employee_Import_Template.xlsx.inspect.ndjson", import.meta.url), "utf8");
...
for (const column of [
  ...
  assert.ok(templateText.includes(column), `${column} must be present in the official template`);
}
...
assert.ok(importSource.includes("Employee code generator is not available. Apply the employee-code migration before importing employees."), "employee import helper fails safely when the employee-code migration is unavailable");
...
for (const deprecated of [
  ...
  assert.ok(!templateText.includes(deprecated), `template should not mention ${deprecated}`);
}
assert.ok(!templateText.includes('"Template Header","Employee Code"'), "template should not expose Employee Code as an import column");
assert.ok(templateText.includes("Employee Code is generated by ERP"), "template should explain generated Employee Code behaviour");
